...
- (void)initView
{
    ...
    NSString *filePath = [path stringByAppendingPathComponent:@"first.plist"];
    NSData *data = [NSData dataWithContentsOfFile:filePath options:0 error:NULL];//从FileName中读取出数据
    //是否第一次进入
    if (!data)
    {
        [self YinDaoPage];
        NSData *data = [@"已进入" dataUsingEncoding:NSUTF8StringEncoding];
...
{
    //引导页图片数组
    NSArray *images =  @[[UIImage imageNamed:@"img_guide_01"],[UIImage imageNamed:@"img_guide_02"],[UIImage imageNamed:@"img_guide_03"],[UIImage imageNamed:@"img_guide_04"]];
    //创建引导页视图
    ...
    [manager POST:requestUrl parameters:parames progress:^(NSProgress * _Nonnull uploadProgress) {
    ...
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
     {
         ...
         if ([[JSON objectForKey:@"success"] integerValue] == 1)
         {
             ...
             //NSString * psw = _psdTextField.text;
             NSUserDefaults * userdefaults = [NSUserDefaults standardUserDefaults];
             [userdefaults setObject:name forKey:@"username"];
             //[userdefaults setObject:psw forKey:@"psw"];
//             NSString *token = [NSString stringWithFormat:@"%@", [[[JSON objectForKey:@"param"] objectForKey:@"user"] objectForKey:@"token"]];
//             NSLog(@"token=%@",token);
//             [userdefaults setObject:token forKey:@"token"];
             [[NSUserDefaults standardUserDefaults]synchronize];
             
//             PersonalCenter * person = [[PersonalCenter alloc]init];
//
