...
@interface YuyueDetail ()
@property (nonatomic, copy) NSMutableArray *datas;

@end

@implementation YuyueDetail
...
- (void)confirm
{
    ...
    parames[@"servers"] = string;
    parames[@"servePerson"] = @"咸国强";
    NSLog(@"parames = %@",parames);
    //url
    NSString *requestUrl = [NSString stringWithFormat:@"%@/api/reserver/addServer",POSTREQUESTURL];
...
- (void)getData
{
    ...
    NSLog(@"parames = %@",parames);
    //url
    NSString *requestUrl = [NSString stringWithFormat:@"%@/api/reserver/newServer",POSTREQUESTURL];
    NSLog(@"requestUrl = %@",requestUrl);
    [manager POST:requestUrl parameters:parames progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        NSNumber *success = [responseObject objectForKey:@"success"];
        if ([success isEqualToNumber:[NSNumber numberWithInt:1]]) {
            NSDictionary *comlist = [[[responseObject objectForKey:@"param"] objectForKey:@"serverPage"] objectForKey:@"categoryList"];
            _datas = [YuyueDetailModel objectArrayWithKeyValuesArray:comlist];
            NSLog(@"总共有%lu",(unsigned long)_datas.count);
