...
{
	if (adate == nil)
		[self loadWithDate:[NSDate date]];//如果未有时间对象，则自己创建并重新调用
	else
	{
		HeavenlyStems = [NSArray arrayWithObjects:@"甲",@"乙",@"丙",@"丁",@"戊",@"己",@"庚",@"辛",@"壬",@"癸",nil];
        EarthlyBranches = [NSArray arrayWithObjects:@"子",@"丑",@"寅",@"卯",@"辰",@"巳",@"午",@"未",@"申",@"酉",@"戌",@"亥",nil];
...
//    NSLog(@"thisdate = %@",thisdate);
	NSDateFormatter *f = [[[NSDateFormatter alloc] init] autorelease];
	[f setDateFormat:@"yyyy-MM-dd"];
	NSDate *startDate = [f dateFromString:start];
	NSDate *endDate = [f dateFromString:end];
//    NSLog(@"f= %@,startDate = %@,endDate = %@",f,startDate,endDate);
...
//    NSLog(@"阴历年lunarYear = %d",lunarYear);
	//计算闰月
	doubleMonth = [self DoubleMonth:lunarYear];
	isLeap = false;
//    NSLog(@"闰月doubleMonth = %d",doubleMonth);
	//计算农历月
	for (lunarMonth = 1; lunarMonth < 12 && sumdays > 0; lunarMonth++)
	{
//        NSLog(@"农历月lunarMonth = %d",lunarMonth);
...
//    NSLog(@"节日 = %@，生肖 = %@",holiday,zodiacLunar);
    
    //单例赋值
    self.yearString = [NSString stringWithFormat:@"%d",lunarYear];
    self.monthString = [NSString stringWithFormat:@"%d",lunarMonth];
    self.dayString = [NSString stringWithFormat:@"%d",lunarDay];
    self.holidayString = [NSString stringWithFormat:@"%@",holiday];
    self.zodiacLunarString = [NSString stringWithFormat:@"%@",zodiacLunar];
