...
//    _imgNext.contentMode = UIViewContentModeScaleAspectFit;
    ...
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.frame.size.width * 0.35, self.frame.size.height - 10 , self.frame.size.width * 0.3, 0)];
...
- (void)reloadImages
{
    ...
    if(_bWebImage)
    {
        [_imgPrev sd_setImageWithURL:[CYSmallTools whetherIsNull:prevImage] placeholderImage:nil];
        [_imgCurrent sd_setImageWithURL:[CYSmallTools whetherIsNull:curImage] placeholderImage:nil];
        [_imgNext sd_setImageWithURL:[CYSmallTools whetherIsNull:nextImage] placeholderImage:nil];
    }
    else
    {
